...
const int TEST_TIME = 12 * 1000; // ie, 3*1000 = 3 seconds
...
void crash_test_all_D()
{
	...
	for (int i = 0; i < BOX_COUNT; i++) {
		for (int j = i + 1; j < BOX_COUNT; j++) {
			if (boxes[i].state != CONTACT_YES || boxes[j].state != CONTACT_YES)
			{
				if (crash_test_D(boxes[i], boxes[j])) {
					boxes[i].state = CONTACT_YES;
					boxes[j].state = CONTACT_YES;
...
{
	//("Test A1", crash_test_all_A1); //TODO: not paying attention to return values. :(
	//("Test A2", crash_test_all_A2);
